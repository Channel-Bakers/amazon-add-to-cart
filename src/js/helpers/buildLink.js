...
export default async (a, sessionID, offerings = false) => {
    try {
        ...
        if (offerings.length) {
            ...
            if (a.hasAttribute('data-component-id') && a.getAttribute('data-component-type') !== 'LinkedImage') {
                ...
                offerings.map(offer => {
                    if (!isEmpty(offer) && offer.asin === ASIN) {
                        ...
                        if (promoCodes.length)
                            a.setAttribute('data-product-promo', JSON.stringify(promoCodes));
                    }
                });
            }
